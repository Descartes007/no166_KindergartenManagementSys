        ...
        <div v-else>
          <el-form-item class="input" label="教师姓名" prop="jiaoshixingming">
            ...
            <el-col :span="24">
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.daibanneirong" label="待办内容" prop="daibanneirong">
                    <span>{{ruleForm.daibanneirong}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `daibanshixiang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.daibanshixiangCrossAddOrUpdateFlag = false;
                  this.parent.search();
